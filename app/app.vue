    ...
    <section class="bg-cover bg-center min-h-screen w-full relative" style="background-image: url('/header.webp')">
        <div class="bg-black/60 absolute top-0 left-0 w-full h-full backdrop-blur-sm">
            <div class="relative z-20 w-full h-full flex items-center justify-center">
                <div
                    class="max-w-lg w-full bg-black/50 backdrop-blur-md border border-white/20 rounded-lg p-6 text-white">
                    <div v-if="!pending" class="w-full">
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-center w-full text-lg font-semibold">{{ server_name }}</p>
                        </div>

                        <div class="grid grid-cols-2 gap-3 my-2">
                            <button class="py-2 px-2 border border-white/10" @click="tab = 'server'"
                                v-bind:class="{ 'border-indigo-600 bg-indigo-500/20 border': tab == 'server' }">Server</button>
                            <button class="py-2 px-2 border border-white/10" @click="tab = 'info'"
                                v-bind:class="{ 'border-indigo-600 bg-indigo-500/20': tab == 'info' }">Info</button>
                        </div>
                        ...
                        <div v-if="tab != 'server'">
                            <ul class="info-panel">
                                ...
                                <li><strong>Map Type:</strong> <span>{{ map_type }}</span></li>
                                <li><strong>Map Seed:</strong> <span>{{ seed }}</span></li>
                                <li><strong>Map Size:</strong> <span>{{ size }}</span></li>
                                <li><strong>Team Limit:</strong> <span>{{ team_limit }}</span></li>
                                <li><strong>Max Population:</strong> <span>{{ max_pop }}</span></li>
                                <li><strong>Timezone:</strong> <span>{{ timezone }}</span></li>
                                <li><strong>Last Wipe:</strong> <span>{{ last_wipe }}</span></li>
                            </ul>
                        </div>
                                ...
                                <div class="flex items-center justify-center py-4">
                                    <div v-if="status == 'online'"
                                        ...
                                        <img class="mr-2 animate-pulse"
                                            src="https://api.iconify.design/majesticons:status-online-line.svg?color=%2345f514"
                                            width="20px">
                                        <span>Online</span>
                                    </div>
                                    <div v-else
                                        class="py-2 px-4 mx-1 border border-red-500 text-red-500 flex items-center">
                                        <img class="mr-2"
                                            src="https://api.iconify.design/radix-icons:cross-circled.svg?color=%23f53100"
                                            width="20px">
                                        <span>Offline</span>
                                    </div>
                                    ...
                                    <div class="border border-white/10 p-2">
                                        <div class="flex flex-col justify-center m-auto">
                                            <div class="flex items-center mb-1 m-auto">
                                                <img src="https://api.iconify.design/solar:ranking-broken.svg?color=%233af86a"
                                                    width="20px">
                            ...
                            <div class="p-3 border border-white/10 mt-3">
                                <div class="grid grid-cols-2 gap-2">
                                    <a href="https://discord.gg/HYnHFYyw" target="_blank" rel="nofollow"
                                        ...
                                        <img src="https://api.iconify.design/flowbite:discord-solid.svg?color=%23ffffff"
                                            width="30">
                                        <strong class="text-lg ml-2">Join Discord</strong>
                                    </a>
                                    <button class="flex items-center justify-center p-2 rounded-lg bg-white"
                                        @click="preview = true">
                                ...
                                <div v-show="preview" class="fixed top-0 left-0 w-full h-full z-10">
                                    <div
                                        class="w-full h-full flex items-center justify-center bg-black/90 backdrop-blur-md relative">
                                        <div class="max-w-[300px] w-full">
                                            <img :src="mapUrl" class="w-full">
                                        </div>
                                        <button class="absolute bottom-2 right-2 bg-red-500 py-1 px-2"
                                            @click="preview = false">Close</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script setup>
const server_name = ref("US Comminuty - 1.5X Vanilla");
...
const type = ref("Modded");
const map_type = ref("Procedural Map");
const seed = ref("4000");
const size = ref("12314");
const last_wipe = ref("2025-09-03T15:36:36.718Z");
const team_limit = ref("4");
